(function(ns) {
	var EscapebusScene = ns.EscapebusScene = Hilo.Class.create({
		Extends: game.BaseScene,
		name: game.configdata.SCENE_NAMES.escapebus,
		skybg:null,
		hill1:null,
		hill2:null,
		road1:null,
		...
		constructor: function(properties) {
			EscapebusScene.superclass.constructor.call(this, properties);
		...
		showLargeHandle:function(){
			this.largebtnbg = new Hilo.Bitmap({
				image:'img/largebtnbg.png',
			}).addTo(this);
			this.largehandle = new Hilo.Bitmap({
				image:'img/largehandle.png',
				x:424+148,
				y:167+91,
				pivotX:148,
				pivotY:91
		...
		layoutBgMap:function(){
			...
			this.terrifiedhead = new Hilo.Bitmap({
				image:'img/terrifiedhead.png',
				visible:false,
			}).addTo(this);
			
			this.busfront = new Hilo.Bitmap({
					image:'img/busfront.png',
					x:650,
					y:126,
					visible:false
			}).addTo(this);
			this.busback = new Hilo.Bitmap({
					image:'img/busbackbody.png',
					x:88,
					y:131,
					visible:false
			}).addTo(this);
			this.busdoor = new Hilo.Bitmap({
					image:'img/busclossdoor.png',
					x:675,
					y:220,
					visible:false
			}).addTo(this);
		...
		showStepTwo:function(){
			...
			this.inbusbg = new Hilo.Bitmap({
				image:'img/inbusbg.png',
			}).addTo(this);
			this.breakwindow = new Hilo.Bitmap({
				image:'img/breakwindow.png',
				rect:[0,163,143,161],
				x:710,
				y:228,
			}).addTo(this);
			this.killfire = new Hilo.Bitmap({
				image:'img/killfire01.png',
				x:1110,
				y:450,
			}).addTo(this);
			this.pipe = new Hilo.Bitmap({
				image:'img/killfire02.png',
				x:1120,
				y:520,
			}).addTo(this);
			this.safehummer = new Hilo.Bitmap({
				image:'img/safehummer.png',
				x:806,
				y:230,
			}).addTo(this);
		...
		showKillfireNote:function(){
			game.drdialog.on(Hilo.event.POINTER_START, function(e) {
				...
				scene.fireuse = new Hilo.Bitmap({
					image:'img/fireuse.png',
				}).addTo(scene);
				scene.fireuse.on(Hilo.event.POINTER_START, function(e) {
					this.removeFromParent();
					game.drdialog.showTxt('img/glass.png');
					game.drdialog.on(Hilo.event.POINTER_START, function(e) {
						game.drdialog.hide();
						game.drdialog.off();
						scene.boyjumpglass();
			...
			game.drdialog.showTxt('img/firetxt.png');
		},
		showOutOpen:function(){
			...
			game.drdialog.showTxt('img/openusetxt.png');
			game.drdialog.on(Hilo.event.POINTER_START, function(e) {
						game.drdialog.hide();
						game.drdialog.off();
		...
		showOutTop:function(){
			new Hilo.Bitmap({
					image:'img/outtop.png',
			}).addTo(this);
			var scene = this;
			game.drdialog.showTxt('img/outtoptxt.png');
			game.drdialog.on(Hilo.event.POINTER_START, function(e) {
						game.drdialog.hide();
						game.drdialog.off();
						console.log('change---');
